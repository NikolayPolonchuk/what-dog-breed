...
from dog_breed_detector.data.dataset import DogDataset, TestDataset
...
def main(cfg: DictConfig):
    ...
    # Создание датасета
    dataset = DogDataset(
        img_path=f"{cfg.paths.data_dir}/{cfg.paths.train_images}",
        csv_path=f"{cfg.paths.data_dir}/{cfg.paths.train_labels}",
        transform=train_transform
    )
    
    # Разделение на train/valid
    train_dataset, valid_dataset = split_dataset(dataset, test_size=cfg.train.test_size)
    ...
    if cfg.get("visualize_samples", True):
        batch_img, batch_label = next(iter(train_dataloader))
        fig = show_samples(
            batch_img, 
            batch_label, 
            num_samples=cfg.get("num_samples_to_show", 8),
            label_idx2name=dataset.label_idx2name,
            channel_mean=channel_mean,
            channel_std=channel_std,
            crop_size=cfg.data.transforms.train.crop
        )
    ...
    # Обучение модели
    train_loss_history, valid_loss_history, train_acc_history, valid_acc_history = train_model(
        net, train_dataloader, valid_dataloader, criterion, optimizer, device, cfg
    )
...
def make_predictions(model, train_dataset, transform_fn, device, cfg):
    ...
    submit_df = pd.read_csv(f"{cfg.paths.data_dir}/{cfg.paths.sample_submission}")
    ...
    test_dataset = TestDataset(
        ...
        img_dir=f"{cfg.paths.data_dir}/{cfg.paths.test_images}",
    ...
    test_dataloader = torch.utils.data.DataLoader(
        ...
        batch_size=cfg.train.test_batch_size if hasattr(cfg.train, "test_batch_size") else 64,
        shuffle=False
    )
